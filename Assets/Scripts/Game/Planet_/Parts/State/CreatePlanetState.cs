...
{
    public record CreatePlanetState : IContinentState
    {
        public void ClickedOnContinent(ContinentMonoBehaviour continent, Camera mainCamera)
        {
            Debug.LogError($"Clicked on a continent while in state {nameof(CreatePlanetState)}. This should not happen");
        }

        public void StartCreatingNewContinent()
        {
            Debug.LogError($"Started creating a new continent while in state {nameof(CreatePlanetState)}. This should not happen");
        }
